...
describe("QuizService", () => {
    ...
    test("should create a new quiz", async () => {
        ...
        quizRepositoryPortMock.save.mockResolvedValue({ id: "quiz1", userId: "user1" });
    ...
    test("should throw error when starting an already started quiz", async () => {
        ...
        await expect(quizService.startQuiz(quiz.id)).rejects.toThrow("Quiz can only be started when pending");
    });


    test("should throw error if quiz not found", async () => {
        await expect(() => quizService.getQuizProgress("nonexistent-id")).toThrow("Quiz not found");
    });
    ...
    test("should not allow multiple quizzes on the same day", async () => {
        ...
        await expect(quizService.createQuiz("user1", { title: "Quiz 1", cards: [card1] }))
    ...
    test("should create a new quiz if not already taken today", async () => {
        ...
        const quiz = await quizService.createQuiz("user1", { title: "Quiz 1", cards: [card1] });
