...
describe("CardController", () => {
    ...
    test("POST /api/cards should create a new card", async () => {
        ...
        cardServiceMock.createCard.mockResolvedValue({ ...newCard, id: "2", category: "FIRST" });
    ...
    test("POST /api/cards should return 400 if invalid data", async () => {
        ...
        const response = await request(app).post("/api/cards").send({ question: "Q3" });
    ...
    test("GET /api/cards/quizz should return 400 on error", async () => {
        cardServiceMock.getCardsForQuiz.mockRejectedValue(new Error("Invalid date"));
    ...
    test("PATCH /api/cards/:cardId/answer should support forceValidation", async () => {
        ...
        const response = await request(app)
            .patch("/api/cards/1/answer")
            .send({ isValid: false, userAnswer: "similar answer", forceValidation: true });
