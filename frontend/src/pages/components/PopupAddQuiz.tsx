...
function PopupAddQuiz({ closePopupCreation } : any) {    
    ...
    const handleInputChange = (id: any, event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    return (
        <Box>
            ...
            <Box className={styles.stylesBoxPopup}>
                    {inputs.map((input) => (
                        <Grid2 sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <Box sx={{ textAlign: 'center' }}>
                <Button onClick={handleRegister} disabled={isDisable()} variant="contained" color="success" sx={{ borderRadius: '14px' }}>
                    Register
                </Button>
            </Box>
